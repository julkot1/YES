    ...
    public static String putArrayToCr(Array array, FileOutputStream out) throws IOException, InvalidYesSyntaxException {
        if(array.getIndex().getArgument() instanceof Value){
            ...
            if(array.getType() == Type.NULL && !array.getToken().equals(SpecialTypeTokens.AR.getToken()))
                throw  new TypeException(array.getLine(),array.getToken(),"to take an element from the array it's type must be given");
        }
        ...
        if(array.getToken().equals(SpecialTypeTokens.AR.getToken())){
            ...
            if(definition.isPresent()){
                ...
                if(anInterface.isPresent()){
                    ...
                    var type = StatementUtils.argumentType(anInterface.get().getArgumentCounts(), NumberUtils.toInt(index));
                    if(type.equals(Type.NULL))throw new TypeException(array.getLine(), array.getToken(), "unknown argument ("+index+" > pta)");
                    array.setType(type);
                }else throw new InvalidYesSyntaxException(array.getLine(), "_STATEMENT "+statement.getToken()+" doesn't have interface (has no arguments)");
            }else throw new InvalidYesSyntaxException(array.getLine(), "ar is only available in _STATEMENT definition");
